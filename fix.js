...
const newHeaderStart = `<nav className="vintage-nav">
  <div className="nav-inner">
    <button 
      ...
      className="logo bg-transparent border-none cursor-pointer text-left"
    >
      <svg className="logo-vinyl" viewBox="0 0 36 36" fill="none">
    ...
    <ul className="nav-links hidden lg:flex">
      <li><button onClick={() => { setCurrentView('home'); setSelectedGenres([]); setSearchQuery(''); }} className={currentView === 'home' ? 'active bg-[var(--pink-light)] !text-[var(--text-main)]' : 'bg-transparent border-none'}>Trang Chủ</button></li>
      <li><button onClick={() => { setCurrentView('tags-list'); setSelectedGenres([]); }} className={currentView === 'tags-list' ? 'active bg-[var(--pink-light)] !text-[var(--text-main)]' : 'bg-transparent border-none'}>Thư Viện</button></li>
      <li><button onClick={() => setCurrentView('rankings-page')} className={currentView === 'rankings-page' ? 'active bg-[var(--pink-light)] !text-[var(--text-main)]' : 'bg-transparent border-none'}>Bảng Xếp Hạng</button></li>
      <li><button onClick={() => setCurrentView('profile')} className={currentView === 'profile' ? 'active bg-[var(--pink-light)] !text-[var(--text-main)]' : 'bg-transparent border-none'}>Hồ Sơ</button></li>
      {currentUser && currentUser.role === 'Admin' && currentView !== 'editor' && currentView !== 'editor-profile' && (
        <li><button onClick={() => setCurrentView('admin')} className="bg-transparent border-none">Admin</button></li>
      )}
      {currentUser && (currentUser.role === 'Editor' || currentUser.role === 'Admin') && (
        <li><button onClick={() => setCurrentView('editor')} className="bg-transparent border-none">Editor</button></li>
      )}
    </ul>
    ...
    <div className="nav-right gap-2 sm:gap-3 flex">
      ...
      <div className="relative hidden md:block w-48">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-[var(--border-strong)]" />
        <input
          type="text"
      ...
      <div className="mini-player">
        ...
        <span className="player-text hidden sm:block">{darkMode ? 'Giao diện Tối' : 'Giao diện Sáng'}</span>
      </div>
      ...
      {currentUser ? (
        <div className="flex items-center gap-2">
            <div 
              ...
              className="flex items-center gap-1.5 cursor-pointer bg-[var(--pink-light)] border border-[var(--pink)] px-3 py-1.5 rounded-full transition-colors"
            >
              ...
              <span className="text-xs font-bold text-[var(--pink-text)] hidden sm:inline-block max-w-[80px] truncate">
                {currentUser.role}
              </span>
            </div>
            ...
            <button
              ...
              className="btn-login py-1 px-3 text-xs"
...
<div className="decor-strip border-y border-[var(--border)] bg-[var(--beige-mid)] dark:bg-[var(--vinyl-dark)] dark:border-gray-800">
...
const headerRegex = /<header className="sticky top-0 z-40 bg-white[\s\S]*?<\/header>/g;
